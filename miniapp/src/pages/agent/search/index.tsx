...
export default function AgentSearch() {
  ...
  const applyFilters = useCallback((items: Listing[], min: string, max: string, beds: number, typeIdx: number) => {
    ...
    if (minP > 0) filtered = filtered.filter(l => l.price >= minP)
    if (maxP > 0) filtered = filtered.filter(l => l.price <= maxP)
    if (beds > 0) filtered = filtered.filter(l => l.beds >= beds)
    const typeName = TYPE_OPTIONS[typeIdx]
    if (typeName !== '不限') {
      const typeVal = TYPE_MAP[typeName]
      if (typeVal) filtered = filtered.filter(l => l.homeType === typeVal)
    }
    return filtered
  }, [])
  ...
  const handleShare = (listing: Listing) => {
    setShareAddress(listing.address || '房源')
  ...
  const selectClient = (client: SimpleClient) => {
    ...
    if (!session) return
    // Copy a share link
    const link = `Estate Epic - ${shareAddress} (经纪人: ${session.displayName})`
